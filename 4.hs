...
checkBoardIsWinner draws board = any id (horizontalBingos ++ verticalBingos)
    where horizontalBingos = map (all (`elem` draws)) board
          verticalBingos = map (all (`elem` draws)) (transpose board)
